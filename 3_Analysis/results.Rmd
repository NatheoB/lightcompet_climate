...
```{r}
output_params_fp <- "../2_Calibration/output/output_params.xlsx"

out_growth <- readxl::read_excel(output_params_fp, sheet = "params_growth", na = "NA")
out_mortality <- readxl::read_excel(output_params_fp, sheet = "params_mortality", na = "NA")

scale_growth <- readxl::read_excel(output_params_fp, sheet = "scale_growth", na = "NA")
scale_mortality <- readxl::read_excel(output_params_fp, sheet = "scale_mortality", na = "NA")

data_shadetol <- readxl::read_excel(output_params_fp, sheet = "shade_tolerance", na = "NA")

data_mean_env <- readxl::read_excel(output_params_fp, sheet = "mean_env", na = "NA") %>% 
...
```{r}
# Species shade tolerance parameters
...
```{r}
# Best SGDD form for both growth and survival
...
comp_sp_gr <- out_growth %>% 
  ...
  dplyr::left_join(data_mean_env %>% dplyr::select(species, n_ind), by = "species") %>% 
...
predict_growth <- function(dbh_mm, lci, sgdd, aet2pet, coefs, scales) {
  ...
  # We squared the standardization of sgdd and we not used sgdd2 directly here as in the formula, we used poly(sgdd, 2) with sgdd scaled
...
predict_mortality <- function(dbh_mm, lci, sgdd, aet2pet, coefs, scales) {
  ...
  sgdd_inv <- ((1/sgdd) - scales$center[scales$var == "sgdd_inv"]) / scales$scale[scales$var == "sgdd_inv"]
  sgdd2 <- ((sgdd - scales$center[scales$var == "sgdd2"]) / scales$scale[scales$var == "sgdd2"]) ^ 2 
  # We squared the standardization of sgdd and we not used sgdd2 directly here as in the formula, we used poly(sgdd, 2) with sgdd scaled
  sgdd <- (sgdd - scales$center[scales$var == "sgdd"]) / scales$scale[scales$var == "sgdd"]
...
data_interspe_gr <- out_growth %>% 
  ...
  dplyr::mutate(
    dD_low = predict_growth(
      dbh_mean, 0.1, sgdd_mean, aet2pet_mean, ., scale_growth),
    dD_high = predict_growth(
      dbh_mean, 0.9, sgdd_mean, aet2pet_mean, ., scale_growth)) %>% 
  dplyr::mutate(lci_effect = log(dD_low/dD_high)) %>%
  dplyr::group_by(species, sample, fold, mod_id, climate_mean, aet2pet_mean, sgdd_mean) %>% 
...
data_interspe_morta <- out_mortality %>% 
  ...
  dplyr::mutate(
    surv_low = 1 - predict_mortality(
      dbh_mean, 0.1, sgdd_mean, aet2pet_mean, ., scale_mortality),
    surv_high = 1 - predict_mortality(
      dbh_mean, 0.9, sgdd_mean, aet2pet_mean, ., scale_mortality)) %>% 
...
plot_interspe_gr_species <- data_interspe_gr %>% 
  ...
  xlab(expression(paste("mean species aridity niche ", aet2pet[mean]))) +
...
data_interspe_morta %>% 
  dplyr::group_by(species) %>% 
  dplyr::summarise(lci_effect_mean = mean(lci_effect),
                   sgdd_mean = mean(sgdd_mean),
                   aet2pet_mean = mean(aet2pet_mean)) %>% 
  dplyr::filter(species %in% c("Larix decidua", "Betula pubescens"))
...
data_intraspe_gr <- out_growth %>% 
  ...
  dplyr::mutate(
    ...
    dD_low_dry = predict_growth(
      dbh_mean, 0.1, sgdd_mean, aet2pet_qt025, ., scale_growth),
    dD_high_dry = predict_growth(
      dbh_mean, 0.9, sgdd_mean, aet2pet_qt025, ., scale_growth),
    lci_effect_dry = dD_low_dry - dD_high_dry,
    
    dD_low_wet = predict_growth(
      dbh_mean, 0.1, sgdd_mean, aet2pet_qt975, ., scale_growth),
    dD_high_wet = predict_growth(
      dbh_mean, 0.9, sgdd_mean, aet2pet_qt975, ., scale_growth),
    lci_effect_wet = dD_low_wet - dD_high_wet,
    
    dD_low_cold = predict_growth(
      dbh_mean, 0.1, sgdd_qt025, aet2pet_mean, ., scale_growth),
    dD_high_cold = predict_growth(
      dbh_mean, 0.9, sgdd_qt025, aet2pet_mean, ., scale_growth),
    lci_effect_cold = dD_low_cold - dD_high_cold,
    
    dD_low_hot = predict_growth(
      dbh_mean, 0.1, sgdd_qt975, aet2pet_mean, ., scale_growth),
    dD_high_hot = predict_growth(
      dbh_mean, 0.9, sgdd_qt975, aet2pet_mean, ., scale_growth),
  ...
  dplyr::mutate(
    ...
    lci_effect_ratio_aet2pet = lci_effect_wet / lci_effect_dry,
    lci_effect_ratio_sgdd = lci_effect_hot / lci_effect_cold,
    
    lci_effect_stdratio_aet2pet = lci_effect_diff_aet2pet / lci_effect_wet,
    lci_effect_stdratio_sgdd = lci_effect_diff_sgdd / lci_effect_wet
  ) %>% 
  ...
  dplyr::summarise(
    ...
    lci_effect_ratio_aet2pet = weighted.mean(lci_effect_ratio_aet2pet, weight),
    lci_effect_ratio_sgdd = weighted.mean(lci_effect_ratio_sgdd, weight),
    
    lci_effect_stdratio_aet2pet = weighted.mean(lci_effect_stdratio_aet2pet, weight),
    lci_effect_stdratio_sgdd = weighted.mean(lci_effect_stdratio_sgdd, weight),
...
data_intraspe_gr_mean <- data_intraspe_gr %>% 
  ...
  dplyr::summarise(
    ...
    lci_effect_ratio_aet2pet_mean = mean(lci_effect_ratio_aet2pet),
    lci_effect_stdratio_aet2pet_mean = mean(lci_effect_stdratio_aet2pet),
    ...
    lci_effect_ratio_sgdd_mean = mean(lci_effect_ratio_sgdd),
    lci_effect_stdratio_sgdd_mean = mean(lci_effect_stdratio_sgdd),
  ...
  dplyr::mutate(
    sign_aet2pet = case_when(
      sign(conf.low_aet2pet) != sign(conf.up_aet2pet) ~ "not significant",
      lci_effect_diff_aet2pet_mean > 0 ~ "wet margin",
      lci_effect_diff_aet2pet_mean < 0 ~ "dry margin"
    ),
    sign_sgdd = case_when(
      sign(conf.low_sgdd) != sign(conf.up_sgdd) ~ "not significant",
      lci_effect_diff_sgdd_mean > 0 ~ "warm margin",
      lci_effect_diff_sgdd_mean < 0 ~ "cold margin"
    ))  %>% 
  dplyr::right_join(data_shadetol, by = "species") %>% 
  dplyr::add_row(species = c("", "AVERAGE SPECIES RESPONSE")) %>% 
  dplyr::mutate(
    species = factor(species, levels = c(as.character(data_mean_env$species), "", "AVERAGE SPECIES RESPONSE")),
    sign_sgdd = factor(sign_sgdd, levels = c("cold margin", "not significant", "warm margin")),
    sign_aet2pet = factor(sign_aet2pet, levels = c("dry margin", "not significant", "wet margin")))
...
data_intraspe_surv <- out_mortality %>% 
  ...
  dplyr::mutate(
    ...
    surv_low_dry = 1 - predict_mortality(
      dbh_mean, 0.1, sgdd_mean, aet2pet_qt025, ., scale_mortality),
    surv_high_dry = 1 - predict_mortality(
      dbh_mean, 0.9, sgdd_mean, aet2pet_qt025, ., scale_mortality),
    lci_effect_dry = surv_low_dry - surv_high_dry,
    
    surv_low_wet = 1 - predict_mortality(
      dbh_mean, 0.1, sgdd_mean, aet2pet_qt975, ., scale_mortality),
    surv_high_wet = 1 - predict_mortality(
      dbh_mean, 0.9, sgdd_mean, aet2pet_qt975, ., scale_mortality),
    lci_effect_wet = surv_low_wet - surv_high_wet,
    
    surv_low_cold = 1 - predict_mortality(
      dbh_mean, 0.1, sgdd_qt025, aet2pet_mean, ., scale_mortality),
    surv_high_cold = 1 - predict_mortality(
      dbh_mean, 0.9, sgdd_qt025, aet2pet_mean, ., scale_mortality),
    lci_effect_cold = surv_low_cold - surv_high_cold,
    
    surv_low_hot = 1 - predict_mortality(
      dbh_mean, 0.1, sgdd_qt975, aet2pet_mean, ., scale_mortality),
    surv_high_hot = 1 - predict_mortality(
      dbh_mean, 0.9, sgdd_qt975, aet2pet_mean, ., scale_mortality),
...
data_intraspe_gr_mean[data_intraspe_gr_mean$species == "AVERAGE SPECIES RESPONSE", 
...
data_intraspe_surv_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE", 
                      c("lci_effect_diff_aet2pet_mean", "conf.low_aet2pet", "conf.up_aet2pet", "sign_aet2pet")] <- 
  list(
...
plot_intraspe_gr_aet2pet <- 
  ...
  geom_vline(xintercept = data_intraspe_gr_mean$lci_effect_diff_aet2pet_mean[data_intraspe_gr_mean$species == "AVERAGE SPECIES RESPONSE"],
             linetype = "dotted",
             linewidth = 0.8,
             colour = ifelse(data_intraspe_gr_mean$lci_effect_diff_aet2pet_mean[data_intraspe_gr_mean$species == "AVERAGE SPECIES RESPONSE"] > 0,
                             "olivedrab3", "salmon")) +
...
plot_intraspe_gr_sgdd <- 
  ...
  geom_vline(xintercept = 0) +
  geom_vline(xintercept = data_intraspe_gr_mean$lci_effect_diff_sgdd_mean[data_intraspe_gr_mean$species == "AVERAGE SPECIES RESPONSE"],
             linetype = "dotted",
             linewidth = 0.8,
             colour = ifelse(data_intraspe_gr_mean$lci_effect_diff_sgdd_mean[data_intraspe_gr_mean$species == "AVERAGE SPECIES RESPONSE"] > 0,
                             "#F1895C", "#82CEF9")) +
  geom_point(size=0.1,alpha=0.6) +
...
plot_intraspe_surv_aet2pet <- 
  ggplot(data = data_intraspe_surv_mean, 
         mapping = aes(y = species, x = lci_effect_diff_aet2pet_mean, color = sign_aet2pet)) +
  geom_vline(xintercept = 0) +
  geom_vline(xintercept = data_intraspe_surv_mean$lci_effect_diff_aet2pet_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"],
             linetype = "dotted",
             linewidth = 0.8,
             colour = ifelse(data_intraspe_surv_mean$lci_effect_diff_aet2pet_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"] > 0,
                             "olivedrab3", "salmon")) +
  geom_point(size=0.1,alpha=0.6) +
  geom_pointrange(aes(xmin = conf.low_aet2pet, xmax = conf.up_aet2pet)) +
  labs(title = "SURVIVAL") +
  ...
  xlim(c(
    min(
      min(data_intraspe_surv_mean$conf.low_aet2pet, na.rm = T), 
      min(data_intraspe_surv_mean$conf.low_sgdd, na.rm = T)),
    max(
      max(data_intraspe_surv_mean$conf.up_aet2pet, na.rm = T),
      max(data_intraspe_surv_mean$conf.up_sgdd, na.rm = T))
  ))
...
plot_intraspe_surv_sgdd <- 
  ggplot(data = data_intraspe_surv_mean, 
         mapping = aes(y = species, x = lci_effect_diff_sgdd_mean, color = sign_sgdd)) +
  geom_vline(xintercept = 0) +
  geom_vline(xintercept = data_intraspe_surv_mean$lci_effect_diff_sgdd_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"],
             linetype = "dotted",
             linewidth = .8,
             colour = ifelse(data_intraspe_surv_mean$lci_effect_diff_sgdd_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"] > 0,
                             "#F1895C", "#82CEF9")) +
  geom_point(size=0.1,alpha=0.6) +
  geom_pointrange(aes(xmin = conf.low_sgdd, xmax = conf.up_sgdd)) +
  labs(title = "SURVIVAL") +
...
limit_x_range_surv <- 0.1
data_intraspe_surv_mean <- data_intraspe_surv_mean %>% 
  dplyr::mutate(conf.up_aet2pet_capped = pmin(conf.up_aet2pet, limit_x_range_surv),
                conf.up_sgdd_capped = pmin(conf.up_sgdd, limit_x_range_surv),
                is_capped_aet2pet = conf.up_aet2pet > limit_x_range_surv,
                is_capped_sgdd = conf.up_sgdd > limit_x_range_surv,
                capped_symbol_aet2pet = if_else(is_capped_aet2pet, "---", ""),
                capped_symbol_sgdd = if_else(is_capped_sgdd, "---", ""))

plot_intraspe_surv_aet2pet_capped <- 
  ggplot(data = data_intraspe_surv_mean, 
         mapping = aes(y = species, x = lci_effect_diff_aet2pet_mean, color = sign_aet2pet)) +
  geom_vline(xintercept = 0) +
  geom_vline(xintercept = data_intraspe_surv_mean$lci_effect_diff_aet2pet_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"],
             ...
             colour = ifelse(data_intraspe_surv_mean$lci_effect_diff_aet2pet_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"] > 0,
                             "olivedrab3", "salmon")) +
  geom_point(size=0.1,alpha=0.6) +
  geom_pointrange(aes(xmin = conf.low_aet2pet, xmax = conf.up_aet2pet_capped)) +
  geom_text(aes(x = limit_x_range_surv, label = capped_symbol_aet2pet)) +
  labs(title = "SURVIVAL") +
  xlab("Difference in light competition effect on tree annual survival\nbetween the species' wet and dry margins") +
  theme_minimal() +
  theme(legend.position = "none",
...
plot_intraspe_surv_sgdd_capped <- 
  ggplot(data = data_intraspe_surv_mean, 
         mapping = aes(y = species, x = lci_effect_diff_sgdd_mean, color = sign_sgdd)) +
  geom_vline(xintercept = 0) +
  geom_vline(xintercept = data_intraspe_surv_mean$lci_effect_diff_sgdd_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"],
             ...
             colour = ifelse(data_intraspe_surv_mean$lci_effect_diff_sgdd_mean[data_intraspe_surv_mean$species == "AVERAGE SPECIES RESPONSE"] > 0,
                             "#F1895C", "#82CEF9")) +
  geom_point(size=0.1,alpha=0.6) +
  geom_pointrange(aes(xmin = conf.low_sgdd, xmax = conf.up_sgdd_capped)) +
  geom_text(aes(x = limit_x_range_surv, label = capped_symbol_sgdd)) +
  labs(title = "SURVIVAL") +
  xlab("Difference in light competition effect on tree annual survival\nbetween the species' warm and cold margins") +
  theme_minimal() +
  theme(legend.position = "none",
...
legend_intraspe_sgdd <- get_legend(
  ggplot(data = data_intraspe_surv_mean %>% 
           ...
           dplyr::mutate(sign_sgdd = case_when(
             ...
             sign_sgdd == "not significant" ~ "not significant"
           ),
           sign_sgdd = factor(sign_sgdd, levels = c("the species' cold margin", "the species' warm margin", "not significant"))),
         mapping = aes(y = species, x = lci_effect_diff_sgdd_mean, color = sign_sgdd)) +
    geom_point(size = 3) +
    theme_minimal() +
    scale_color_manual(values=c("#82CEF9", "#F1895C", "gray")) +
  ...
  ggplot(data = data_intraspe_surv_mean %>% 
           ...
           dplyr::mutate(
             sign_aet2pet = case_when(
               ...
               sign_aet2pet == "not significant" ~ "not significant"
             ),
             sign_aet2pet = factor(sign_aet2pet, levels = c("the species' dry margin", "the species' wet margin", "not significant"))),
         mapping = aes(y = species, x = lci_effect_diff_aet2pet_mean, color = sign_aet2pet)) +
    geom_point(size = 3) +
    theme_minimal() +
    scale_color_manual(values=c("salmon", "olivedrab3", "gray")) +
    theme(legend.position = "top") +
    guides(color = guide_legend(title = "light competition effect is greater in :",
                                title.theme = element_text(vjust = 0, size = 11),
                                title.vjust = 0,
...
# FINAL PLOTS - CAPPED (for manuscript)
cowplot::plot_grid(
  cowplot::ggdraw() + cowplot::draw_label("Temperature gradient (sgdd)", size = 14, fontface = "bold"),
  legend_intraspe_sgdd,
  cowplot::plot_grid(
    plot_intraspe_gr_sgdd, NULL, plot_intraspe_sp, NULL, plot_intraspe_surv_sgdd_capped,
...
cowplot::plot_grid(
  cowplot::ggdraw() + cowplot::draw_label("Aridity gradient (aet2pet)", size = 14, fontface = "bold"),
  legend_intraspe_aet2pet,
  cowplot::plot_grid(
    plot_intraspe_gr_aet2pet, NULL, plot_intraspe_sp, NULL, plot_intraspe_surv_aet2pet_capped,
    nrow = 1, rel_widths = c(15, 1, 3, 1, 15)
  ),
  ncol = 1, rel_heights = c(1, 3, 15)
)

# FINAL PLOTS - UNCAPPED (FOR APPENDIX)
cowplot::plot_grid(
  cowplot::ggdraw() + cowplot::draw_label("Temperature gradient (sgdd)", size = 14, fontface = "bold"),
  legend_intraspe_sgdd,
  cowplot::plot_grid(
...
plot_gr_pisy_sgdd <- out_growth %>% 
  ...
  dplyr::mutate(x_sgdd = rep(seq(unique(sgdd_qt025), unique(sgdd_qt975),
                ...
                lci_group = rep(c("in light", "in shade"), times = 100),
                
                lci_value = case_when(
                  lci_group == "in light" ~ 0.1,
                  lci_group == "in shade" ~ 0.9
                ),
                lci_group = factor(lci_group, c("in light", "in shade"))) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(
    dD = predict_growth(
      dbh_mean, lci_value, x_sgdd, aet2pet_mean, ., scale_growth)) %>% 
  dplyr::group_by(species, sample, fold, 
                  x_sgdd, lci_group) %>% 
...
plot_surv_pisy_sgdd <- out_mortality %>% 
  ...
  dplyr::mutate(
    surv = 1 - predict_mortality(
      dbh_mean, lci_value, x_sgdd, aet2pet_mean, ., scale_mortality)) %>% 
  ...
  scale_y_continuous(limits = c(0.988, 1),
                     breaks = seq(0.988, 1, by = 0.002)) +
  scale_x_continuous(limits = c(1000, 2600),
                     breaks = seq(1000, 2500, by = 500))
...
plot_gr_pisy_aet2pet <- out_growth %>% 
  ...
  dplyr::mutate(
    dD = predict_growth(
      dbh_mean, lci_value, sgdd_mean, x_aet2pet, ., scale_growth)) %>% 
  dplyr::group_by(species, sample, fold, 
                  x_aet2pet, lci_group) %>% 
  dplyr::summarise(dD = weighted.mean(dD, weight)) %>% 
...
# FINAL PLOT
cowplot::plot_grid(
  ggdraw() + draw_label("Average species response", fontface = "bold"),
  cowplot::plot_grid(
